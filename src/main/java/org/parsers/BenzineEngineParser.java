...
public class BenzineEngineParser implements EngineParser {
    private final List<Double> possibleCC = new ArrayList<>(Arrays.asList(1000.0, 2000.0, 3000.0, 4000.0, 5000.0, 6000.0, 8000.0));
    private final List<Double> possibleKwWithoutTurbo = new ArrayList<>(Arrays.asList(55.0,147.0, 245.0, 253.0, 331.0, 510.0, 736.0));
    private final List<Double> possibleBenzinKWwithTurbo = new ArrayList<>(Arrays.asList(71.5,191.1,318.5,328.0,430.3, 663.0, 956.8));
     ...
     * If no information is provided for power/displacement and emission standard - default values are set (respectively - 2L and euro4).
    ...
    public List validateEngineInput(String engineInput) {
        ...
        String regex = "^engine=(?:(B)(?:-(\\d+(?:hp|L)(?:T?))-(euro[3-6]))?)$";

        Pattern pattern = Pattern.compile(regex);
        Matcher matcher = pattern.matcher(engineInput);
        try {
            if(matcher.find()) {
                engineType = matcher.group(1); // content: always B
    ...
    private boolean checkValidityOfPowerEntered(String displacementOrPower, String number) {
        ...
        if (displacementOrPower.equals("hp")) {
            ...
            if(fallsWithinKwRangeFlag){
                ...
                throw new IllegalArgumentException("Not possible input for hp and Benzine");
            }
            ...
            for (double ccPossible : possibleCC) {
                ...
                throw new IllegalArgumentException("Not possible input for L and Benzine");
            }
